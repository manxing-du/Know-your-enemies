...
from marl_bidding.data_loader import IPinyouDataLoader
from marl_bidding.agent.ddpg.ddpg_ma import DDPGAgent
from marl_bidding.env.MADDPGEnv import MultiAgentDDPG
...
def main(args):
    ...
    if train_mode:
        ...
        if om:
            if not random_om:
                ...
                for i in range(agent_num):
                    market_file.append('../predictions/agent_{}/{}_{}'.format(i, camp, 'train') + file_prefix + '.txt')
        ...
        if om:
            if not random_om:
                for i in range(agent_num):
                    market_file.append('../predictions/agent_{}/{}_{}'.format(i, camp, 'test') + file_prefix + '.txt')

    a_op = np.arange(0., max_bid+1, 1.) / max_bid

    bid_log_path = '../log/bid_logs/marl/train_{}'.format(train_mode)
    ...
    for z in range(agent_num):
        bid_log.write('{}{}\t'.format('ddpg_bid_', z))
    bid_log.write('{}\t'.format('click'))
    bid_log.write('{}\n'.format('market'))

    data_loader_list = []
    for i in range(agent_num):
        if len(market_file) > 1:
            data_loader = IPinyouDataLoader(data_path='../data/make-ipinyou-data/', market_path=market_file[i],
    ...
    env = MultiAgentDDPG(agent_num=agent_num,
    ...
    exp_name = 'om_{}'.format(om)
    agents = []

    # save the ddpg model for each agent
    model_path = MODEL_DIR + '/marl/{}/{}/{}/{}'.format(camp, c0, om, random_om)
    ...
    for i in range(agent_num):
        ...
        # learning log per agent
        output_dir = LOG_DIR + '/experiments/marl/agent_{}/{}/reward_{}/{}/train_{}/random_om{}_c0_{}_q_{}_a_{}_s_{}_n_' \
                               '{}_comp_{}_{}'.format(i, exp_name, reward_type, camp, train_mode, int(random_om), str(c0),
                                                      q_type, agent_num, seed, noise, compete_mode, time.time())
        if not osp.exists(output_dir):
            os.makedirs(output_dir)

        logger_kwargs_i = {'output_dir': output_dir, 'exp_name': exp_name}

        model_path_i = model_path + 'agent_{}/'.format(i)
        if not osp.exists(model_path_i):
            os.makedirs(model_path_i)

        model_name = model_path_i + 'model' + file_prefix + '_om_{}'.format(om)
        scope = 'agent_{}'.format(i)

        ddpg_agent = DDPGAgent(env.observation_space, env.action_space, q_type, args['train_mode'], model_path_i,
                               model_name, scope, batch_size, seed=seed, market_price_dim=m_dim, om=om, logger_kwargs=logger_kwargs_i)
        agents.append(ddpg_agent)
    ...
    for t in range(int(total_steps)):
        ...
        if t > start_steps:
            ...
            for i, agent in enumerate(agents):
                # if (i == 0) & om:
                #    action = agent.act_om(obses[i], info.get('x_market'))
                # else:
                action = agent.act(obses[i])[0]
        ...
        for i in range(agent_num):
            if type(scaled_bid[i]) is np.ndarray:
                ddpg_bid = scaled_bid[i][0]
            else:
                ddpg_bid = scaled_bid[i]

            # record the ddpg_bid and market price
        ...
        for i, agent in enumerate(agents):
            if type(agent) is DDPGAgent:
                agent.replay_buffer.store(obses[i], actions[i], r[i], obses2[i], d,
                                          info['agent_' + str(i)].get('x_market', None),
                                          info['agent_' + str(i)].get('x2_market', None), a_op)
