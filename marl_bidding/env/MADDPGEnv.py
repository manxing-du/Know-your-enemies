...
class MultiAgentDDPG(RTBBaseEnv):
    ...
    def __init__(self, agent_num, data_loader, om=True, random_om=True, obs_noise_level=0., market_price_dim=10, init_budget=100000.,
                 reward_schedule=1, max_bid_price=300., max_episode_steps=1000, episode_steps_ratio=0., compete_mode=1, seed=0):
        ...
        self.dataset_length = self.data_loader[0].get_dataset_length()
    ...
    def _get_market_p(self):
        # market_price = self.bid.get('market_price', np.random.uniform(0., 1., self.market_price_dim))
    ...
    # def _get_opponent_action(self, actions, x_market):
    #     # print(actions[0])
    #     a_op = self.a_op
    #
    #     # take the ddpg bid
    #     # print([n for n, i in enumerate(a_op) if i > actions[0]])
    #
    #     try:
    #         lose_index = [n for n, i in enumerate(a_op) if i > actions[0]][0]
    #         x_market[lose_index:] = [0.] * (len(x_market) - lose_index)
    #     except IndexError:
    #         # a-i = 1
    #         x_market = [0.] * len(x_market)
    #
    #     return x_market
    ...
    def step(self, actions, compete_mode):
        ...
        # if done:
        #     print(self.agent_budget_left)
        #     if self.episodes >=10:
        #         sys.exit()

        info = {}
        for i in range(self.agent_num):
            self.bid_list['agent_' + str(i)] = self._get_next_bidding_agent(compete_mode, i)
            # self.bid = self._get_next_bidding_agent(compete_mode, i)
            self.bid = self.bid_list['agent_' + str(i)]
            x_market = self._get_market_p()
            info['agent_' + str(i)] = {'x_market': x_market}
        ...
        for i in range(self.agent_num):
            ...
            info['agent_' + str(i)]['x2_market'] = x2_market

        obs = self._convert_bid_to_obs(self.bid)
        # op_dist = self._get_opponent_action(actions, x_market)
        return obs, rewards, done, info, price_paid, scaled_bid, self.bid['click']
    ...
    def _render(self, mode='human', close=False):
        if close:
            return
        outfile = sys.stdout
        outfile.write('pCVR: {}, price paid: {}, is conversion: {},bid price: {}, reward: {}'.format(
            self.last_bid['estimated_cvr'],
